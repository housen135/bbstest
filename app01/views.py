...
def my_custom_sql():
    with connection.cursor() as cursor:
        a='图样图森破'
        cursor.execute('SELECT * FROM app01_blog where site_title = %s or id =4',[a])
        print('description',cursor.description)
        columns = [col[0] for col in cursor.description]
        print('col',columns)
        row2 = cursor.fetchall()
        print(row2)
        print(dict(zip(columns,row2)))
...
def home(request):
    ...
    page_html = mark_safe(page_obj.page_html())
    a=my_custom_sql()
    print('sql',a)
...
def get_UpAndDown(request):
    if request.is_ajax():
        ...
        if request.method == 'POST':
            ...
            # print('是否登录',request.user.is_authenticated())
...
def back_set(request):
    ...
    print('css=======>',user_blog.site_theme)
